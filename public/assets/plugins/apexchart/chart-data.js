...
if ($('#revenue-chart2').length > 0) {
	...
	var expenseData = [61500, 66200, 72400, 76850, 80300, 78340];
	var incomeData = [95000, 98000, 105000, 110000, 118000, 124500];
	var remainingData = incomeData.map(function (val, index) {
		return val - expenseData[index];
	});
	...
	var options = {
		...
		fill: {
			...
			gradient: {
				type: 'vertical',
				shadeIntensity: 0,
				gradientToColors: ['#FB7185', '#86EFAC'],
				opacityFrom: 1,
				opacityTo: 1,
				stops: [0, 100]
			}
		...
		tooltip: {
			custom: function ({ series, dataPointIndex, w }) {
				...
				var expenseValue = series[0][dataPointIndex];
				var balanceValue = series[1][dataPointIndex];

				return '<div style="background:#ffffff;padding:20px 24px;border-radius:20px;box-shadow:0 25px 50px rgba(0,0,0,0.08);min-width:180px;">' +
					'<div style="font-size:18px;font-weight:600;color:#111827;margin-bottom:12px;">' + month + '</div>' +
					'<div style="display:flex;align-items:center;font-size:15px;color:#6B7280;margin-bottom:6px;"><span style="width:8px;height:8px;background:#EF4444;border-radius:50%;display:inline-block;margin-right:8px;"></span>Expenses</div>' +
					'<div style="font-size:22px;font-weight:700;color:#111827;margin-bottom:10px;">Rs. ' + expenseValue.toLocaleString() + '</div>' +
					'<div style="display:flex;align-items:center;font-size:15px;color:#6B7280;margin-bottom:6px;"><span style="width:8px;height:8px;background:#22C55E;border-radius:50%;display:inline-block;margin-right:8px;"></span>Balance</div>' +
					'<div style="font-size:18px;font-weight:700;color:#111827;">Rs. ' + balanceValue.toLocaleString() + '</div>' +
					'</div>';
			}
		},
		responsive: [{
...
if ($('#growth-trend').length > 0) {
	var incomeTrend = [95000, 98000, 105000, 110000, 118000, 124500, 121000, 126500, 129000, 131000, 128000, 135000];
	var expenseTrend = [61500, 66200, 72400, 76850, 80300, 78340, 81200, 83500, 79950, 84200, 81500, 86900];
	var options = {
		chart: {
			type: 'area',
			height: 350,
		...
		yaxis: {
			...
			labels: {
				formatter: function (val) {
					return 'Rs. ' + Math.round(val / 1000) + 'k';
				},
				style: {
					colors: '#707070',
					fontSize: '12px'
...
if ($('#region-wise-growth').length > 0) {
	...
	var options = {
		...
		labels: ['Food', 'Rent', 'Bills', 'Travel', 'Shopping', 'Others'],
		colors: ['#5B6EF5', '#60C28E', '#F5A544', '#1FBAD6', '#8C7AE6', '#EF4444'],
		stroke: {
			width: 0
		},
		plotOptions: {
